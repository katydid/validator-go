...
func (this *errWrongType) Error() string {
	return fmt.Sprintf("validator/parser: expected %s, but got %#v", this.typ, this.res)
}

// ParseGrammar parses a validator grammar and returns an abstract syntax tree.
func (this *Parser) ParseGrammar(s string) (*ast.Grammar, error) {
	...
	if !ok {
		return nil, &errWrongType{"*validator.Grammar", g}
	}
	return gram, nil
}
